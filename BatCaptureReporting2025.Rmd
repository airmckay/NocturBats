...
## Latin name for each bat species
...
bcap3 <- bcap2 %>% 
  ...
  mutate(taxa = factor(case_when(
    Species == "PPYG" ~ "Pipistrellus pygmaeus", 
    Species == "PNAT" ~ "Pipistrellus nathusii",
    Species == "MDAU" ~ "Myotis daubentonii",
    Species == "ENIL" ~ "Eptesicus nilssonii"
  )))
...
nets3 <- nets2 %>% dplyr::filter(
  siteID %in% c('MN-MO', 'MN-LS', 'MN-SO', 'MN-NM', "MN-LI")) %>%
...
## clean up date info 
## Change the date format so that it is in the format the folks at Stavanger museum AND Arts obs prefer
...
## I am going to add the MN site info separately in excel - I promise to come up with a better solution before next season! 
...
## After doing much cleaning in excel and submitting data to ArtsObs, collecting other observations that Mara had submitted as well - we now have all bat captures combined for 2025. This was unfortuntately not so frictionless this year - especially since there was a mix of paper and digital datasheets. 

```{r}

bats25 <- read_csv("C:/Users/apmc/OneDrive - Norwegian University of Life Sciences/BatLab Norway/Projects/Noctur/Data/ReportingBatCaptures/2025/MJD_BatCaptures_2025.csv")
# 159 obs of 22 vars

bats25 <- bats25 %>% mutate(Species = factor(Species), 
                            Sex = factor(Sex), 
                            Age = factor(Age),
                            BandNumber = factor(BandNumber), 
                            ReproductiveStatus = factor(ReproductiveStatus), 
                            CaptureMethod = factor(`Capture method`), 
                            Locality = factor(Locality), 
                            Fylke = factor(Fylke)) 

bats25$date <- format(bats25$Date, format = "%d.%m.%Y")
levels(bats25$BandNumber)

mn <- bats25 %>% filter(CaptureMethod=="Mist net") 
bb <- bats25 %>% filter(CaptureMethod=="Bat box") %>% droplevels()

ggplot(mn) + 
  geom_bar(aes(x=Species)) + 
  facet_wrap(~Fylke, nrow=2) + 
  theme_bw(base_size = 18) + 
  ylab("") 
  ggtitle("Bats captured during mist net surveys in 2025")


ggplot(bb) + 
  geom_bar(aes(x=Fylke)) + 
  facet_wrap(~Species) + 
  theme_bw(base_size = 18) + 
  ylab("") 
  ggtitle("Bats captured during bat box surveys in 2025")
  
table(bats25$Species, bats25$CaptureMethod)
  
b25 <- st_as_sf(bats25, coords=c("Longitude", "Latitude"), crs = "+proj=latlong +ellps=WGS84 +datum=WGS84 +units=m +no_defs")
143/159
crs(b25$geometry)

# plot(boxdf2)

mapview(b25, zcol = "CaptureMethod")
